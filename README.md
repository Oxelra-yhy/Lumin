<p align="center">
  <img src="docs/assets/lumin-logo-new.png" alt="Lumin" width="128" />
</p>

<h1 align="center"><em>Lumin</em></h1>

<p align="center">
  <strong>面向人类研究人员和 Agent 的开放式科学知识专著。</strong>
</p>

<p align="center">
  <a href="https://oxelra-yhy.github.io/Lumin/">在线站点</a>
  ·
  <a href="https://oxelra.com/">Oxelra</a>
</p>

## 项目定位

Lumin 是 Oxelra 的开放式科学知识专著项目，面向人类研究人员和 Agent。它以专著体量组织研究主题，开放预览页面、PDF 草稿、LaTeX 源稿、目录、版本记录和 Issues 审议通道，让阅读、引用、复核与修订都有清晰路径。

这个项目不把 Agent 参与生成的草稿包装成最终出版物，而是把书稿作为可讨论、可追踪、可持续修订的研究材料开放出来。人类研究人员可以深入阅读，Agent 可以进行结构化理解、引用与复核。

## 书稿

### Topological Photonics from Maxwell's Equations

从 Maxwell 方程出发组织拓扑光子学的基础框架，覆盖模式理论、Bloch 能带、Berry 曲率、Chern 数、边界态、体边对应，并延伸至 Floquet 系统、Weyl 点、合成维度和非线性量子边界通道。

[查看仓库](https://github.com/Oxelra-yhy/Topological-Photonics)

### Non-Hermitian Photonics

以开放光学系统为主线，讨论损耗、增益、色散、辐射边界和散射极点如何改变本征模图景，并展开准正规模、异常点、PT 对称、非布洛赫拓扑、趋肤效应、BIC 与器件物理边界。

[查看仓库](https://github.com/Oxelra-yhy/Non-Hermitian)

两部书稿目前均处于公开审议前草稿阶段。读者可以通过对应仓库的 Issues 提交概念跳跃、推导疑点、引用线索、图表问题和实验可复现性反馈。

## 阅读与审议

Lumin 适合以“研究共同体预审”的方式阅读：先把书稿视为可讨论、可定位、可修正的工作稿，再围绕具体章节提出反馈。

尤其欢迎以下类型的 Issues：

- 章节结构是否清晰，概念过渡是否自然
- 公式推导、符号约定和物理假设是否一致
- 图表、数值流程和实验描述是否足够可复现
- 关键文献、历史脉络和术语边界是否需要补充

Lumin 的目标是保持入口轻量、来源明确、状态透明，让开放式科学知识专著从生成、复核到公开审议的每一步都能被人类研究人员和 Agent 看见。
